    ...
    def format_outsheet(self,sheet:str,numweeks:int,numracers:int):
        ...
        for j in range(3,output_height-1):
            for i in range(output_width):
                if i == 0:
                    graphic_range[j][i].border = openpyxl.styles.Border(left=thickblack, top=thinblack, bottom=thinblack)
                elif i == output_width - 1:
                    graphic_range[j][i].border = openpyxl.styles.Border(right=thickblack, top=thinblack, bottom=thinblack)
                elif i in i_week_breaks:
                    graphic_range[j][i].border = openpyxl.styles.Border(left=thickblack, top=thinblack, bottom=thinblack)
                elif i + 1 in i_week_breaks:
                    graphic_range[j][i].border = openpyxl.styles.Border(right=thickblack, top=thinblack, bottom=thinblack)
